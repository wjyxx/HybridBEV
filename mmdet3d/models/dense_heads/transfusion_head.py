...
class PositionEmbeddingLearned(nn.Module):
    ...
    def forward(self, xyz):
        """ 
            xyz : 1 L 2
        """
        xyz = xyz.transpose(1, 2).contiguous()  # 1 2 L
        position_embedding = self.position_embedding_head(xyz)  # 1 c_out L
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, query, key, query_pos, key_pos, attn_mask=None):
        ...
        if self.self_posembed is not None:
            query_pos_embed = self.self_posembed(
                query_pos).permute(2, 0, 1)   # L 1 c_out
        else:
        ...
        query = query.permute(2, 0, 1)  # q_l bsz c
        ...
        # NxCxP to PxNxC
        query = query.permute(1, 2, 0)  # q_l bsz c -> bsz c q_l
        return query
...
class TransFusionHead(nn.Module):
    def __init__(self,
        ...
        if self.initialize_by_heatmap:
            ...
            # query feature
            self.query_feat = nn.Parameter(torch.randn(
                1, hidden_channel, self.num_proposals))
            self.query_pos = nn.Parameter(torch.rand(
                [1, self.num_proposals, 2]), requires_grad=learnable_query_pos)
        ...
        for i in range(self.num_decoder_layers):
            ...
            self.prediction_heads.append(FFN(
                hidden_channel, heads, conv_cfg=conv_cfg, norm_cfg=norm_cfg, bias=bias))  # 执行 anchor-free预测头
        ...
        if self.fuse_img:
            ...
            # deepcopy heatmaphead of lidar bev
    ...
    def create_2D_grid(self, x_size, y_size):
        """ create 2d grid
        Args:
            x_size (int): length of width.
            y_size (int): length of heigh.
        Return:
            grid_size (tensor): size of (1, x_size*y_size, 2), type of 2d grid.
        """
        ...
        coord_base = coord_base.view(1, 2, -1).permute(0, 2, 1)  # 1 L 2
        return coord_base
    ...
    def forward_single(self, inputs, img_inputs, img_metas):
        ...
        Args:
            inputs (torch.Tensor): Input feature map with the shape of
                [B, 512, 128(H), 128(W)]. (consistent with L748)  LiDAR BE
        ...
        if self.fuse_img:
            img_feat = self.shared_conv_img(
                img_inputs)  # [BS * n_views, C, H, W]

            img_h, img_w, num_channel = img_inputs.shape[-2], img_inputs.shape[-1], img_feat.shape[1]
            raw_img_feat = img_feat.view(batch_size, self.num_views, num_channel, img_h, img_w).permute(
                0, 2, 3, 1, 4)  # [BS, C, H, n_views, W]
            img_feat = raw_img_feat.reshape(
                batch_size, num_channel, img_h, img_w * self.num_views)  # [BS, C, H, n_views*W]
            ...
            if self.img_feat_collapsed_pos is None:
                ...
                img_feat_collapsed_pos = self.img_feat_collapsed_pos  # 1 L 2

            bev_feat = lidar_feat_flatten
            for idx_view in range(self.num_views):
                bev_feat = self.decoder[2 + idx_view](bev_feat, img_feat_collapsed[..., img_w * idx_view:img_w * (
                    idx_view + 1)], bev_pos, img_feat_collapsed_pos[:, img_w * idx_view:img_w * (idx_view + 1)])

        #################################
        # image guided query initialization
        #################################
        if self.initialize_by_heatmap:
            # lidar bev feature to heatmap(centerpoint style)
            ...
            # get top num_proposals heatmap num
            top_proposals = heatmap.view(
                batch_size, -1).argsort(dim=-1, descending=True)[..., :self.num_proposals]
            # classifier every pixel in C channel and num_proposals  w.r.t. C*num_proposals
            top_proposals_class = top_proposals // heatmap.shape[-1]
            # bs * num_proposals
            top_proposals_index = top_proposals % heatmap.shape[-1]
            # expand -1 means not changing the size of that dimension    ;  for gather ,the dim used is -1 for the third dimension ,cause the third dimension reprensente for the h*w ,and which is also the proposals heatmap maxpooling operation
            ...
            query_feat = self.query_feat.repeat(
                batch_size, 1, 1)  # [BS, C, num_proposals]
        ...
        for i in range(self.num_decoder_layers):
            prefix = 'last_' if (
                i == self.num_decoder_layers - 1) else f'{i}head_'
            ...
            # query_pos: b hw 2 -> b 2 hw # 计算 学习的 偏移中心
        ...
        if self.fuse_img:
            ...
            img_feat = raw_img_feat.permute(
                0, 3, 1, 2, 4)  # [BS, n_views, C, H, W]
            img_feat_flatten = img_feat.view(
                batch_size, self.num_views, num_channel, -1)  # [BS, n_views, C, H*W]
            ...
            # create new container for img query feature
            query_feat = torch.zeros_like(query_feat)
            query_pos_realmetric = query_pos.permute(
                0, 2, 1) * self.test_cfg['out_size_factor'] * self.test_cfg['voxel_size'][0] + self.test_cfg['pc_range'][0]
            query_pos_3d = torch.cat(
                [query_pos_realmetric, res_layer['height']], dim=1).detach().clone()
            if 'vel' in res_layer:
            ...
            on_the_image_mask = torch.ones(
                [batch_size, self.num_proposals]).to(query_pos_3d.device) * -1
            ...
            for sample_idx in range(batch_size if self.fuse_img else 0):
                ...
                img_flip = img_metas[sample_idx]['flip'] if 'flip' in img_metas[sample_idx].keys(
                ) else False
                img_crop_offset = (
                    query_pos_3d.new_tensor(
                        img_metas[sample_idx]['img_crop_offset'])
                ...
                query_pos_3d_with_corners = torch.cat([query_pos_3d[sample_idx], boxes.corners.permute(
                    2, 0, 1).view(3, -1)], dim=-1)  # [3, num_proposals] + [3, num_proposals*8]
                ...
                if batch_size == 1:  # skip during inference to save time
                    ...
                    points = apply_3d_transformation(
                        query_pos_3d_with_corners.T, 'LIDAR', img_metas[sample_idx], reverse=True).detach()
                num_points = points.shape[0]

                for view_idx in range(self.num_views):
                    pts_4d = torch.cat(
                        [points, points.new_ones(size=(num_points, 1))], dim=-1)
                    ...
                    coor_x, coor_corner_x = coor_x[0:self.num_proposals,
                                                   :], coor_x[self.num_proposals:, :]
                    coor_y, coor_corner_y = coor_y[0:self.num_proposals,
                                                   :], coor_y[self.num_proposals:, :]
                    coor_corner_x = coor_corner_x.reshape(
                        self.num_proposals, 8, 1)
                    coor_corner_y = coor_corner_y.reshape(
                        self.num_proposals, 8, 1)
                    coor_corner_xy = torch.cat(
                        [coor_corner_x, coor_corner_y], dim=-1)

                    h, w = img_pad_shape
                    on_the_image = (coor_x > 0) * (coor_x < w) * \
                        (coor_y > 0) * (coor_y < h)
                    ...
                    center_xs = (coor_x[on_the_image] /
                                 self.out_size_factor_img)
                    # center on the feature map
                    centers = torch.cat([center_xs, center_ys], dim=-1).int()
                    corners = (coor_corner_xy[on_the_image].max(
                        1).values - coor_corner_xy[on_the_image].min(1).values) / self.out_size_factor_img
                    ...
                    gaussian_mask = (-distance /
                                     (2 * sigma[:, None] ** 2)).exp()
                    gaussian_mask[gaussian_mask <
                                  torch.finfo(torch.float32).eps] = 0
                    attn_mask = gaussian_mask

                    query_feat_view = prev_query_feat[sample_idx,
                                                      :, on_the_image]
                    query_pos_view = torch.cat([center_xs, center_ys], dim=-1)
                    # ==========decode for image fusion feature , the query-pos integrated the position from 3d space=========
                    query_feat_view = self.decoder[self.num_decoder_layers](
                        query_feat_view[None], img_feat_flatten[sample_idx:sample_idx + 1, view_idx], query_pos_view[None], img_feat_pos, attn_mask=attn_mask.log())
                    query_feat[sample_idx, :,
                               on_the_image] = query_feat_view.clone()

            self.on_the_image_mask = (on_the_image_mask != -1)
            # the second prediction with image as kv lidar as q
            ...
            for key, value in res_layer.items():
                ...
                res_layer[key][~self.on_the_image_mask.unsqueeze(1).repeat(
                    1, pred_dim, 1)] = first_res_layer[key][~self.on_the_image_mask.unsqueeze(1).repeat(1, pred_dim, 1)]
            ret_dicts.append(res_layer)  # [4,...]
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        for idx_layer in range(self.num_decoder_layers if self.auxiliary else 1):
            ...
            layer_cls_score = layer_score.permute(
    ...
    def get_bboxes(self, preds_dicts, img_metas, img=None, rescale=False, for_roi=False):
        ...
        for layer_id, preds_dict in enumerate(preds_dicts):
            ...
            batch_score = batch_score * \
                preds_dict[0]['query_heatmap_score'] * one_hot
            ...
            for i in range(batch_size):
                ...
                if self.test_cfg['nms_type'] != None:
                    ...
                    ret = dict(
                        bboxes=boxes3d[keep_mask], scores=scores[keep_mask], labels=labels[keep_mask])
                else:  # no nms
