...
class CenterPoint_Distill(MVXTwoStageDetector):
    ...
    def __init__(
        ...
        super(CenterPoint_Distill, self).__init__(
            pts_voxel_layer,
            pts_voxel_encoder,
            pts_middle_encoder,
            pts_fusion_layer,
    ...
    def extract_pts_feat(self, pts, img_feats, img_metas):
        ...
        ret_lowlevel_feats = None
        ret_multiscale_feats = None

        x, low_ms_voxel_feats = self.pts_middle_encoder(
            voxel_features, coors, batch_size
        )
        ret_lowlevel_feats = x

        x = self.pts_backbone(x)
        ret_multiscale_feats = x

        if self.with_pts_neck:
            x = self.pts_neck(x)

        return x, low_ms_voxel_feats, ret_lowlevel_feats, ret_multiscale_feats

    def extract_feat(self, points, img, img_metas):
        """Extract features from images and points."""
        ret_dict_feats = {}
        img_feats = self.extract_img_feat(img, img_metas)
        pts_feats, low_ms_voxel_feats, ret_lowlevel_feats, ret_multiscale_feats = (
            self.extract_pts_feat(points, img_feats, img_metas)
        )
        ret_dict_feats.update(
            {
                "low_ms_voxel_feats": low_ms_voxel_feats,
                "low_bev_feats": ret_lowlevel_feats,
                "high_ms_feats": ret_multiscale_feats,
            }
        )  # for simple test low voxel distill
        return (img_feats, pts_feats, ret_dict_feats)
