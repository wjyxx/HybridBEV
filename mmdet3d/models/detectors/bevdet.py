...
class BEVDetSequential(BEVDet):
    ...
    def extract_img_feat(self, img, img_metas):
        ...
        for img, rot, tran, intrin, post_rot, post_tran in zip(
            ...
            geom = self.img_view_transformer.get_geometry(
                rot, tran, intrin, post_rot, post_tran
            )
            depth = self.img_view_transformer.get_depth_dist(
                x[:, : self.img_view_transformer.D]
            )
            img_feat = x[
                :,
            ...
            volume = volume.view(
                B,
                N,
                self.img_view_transformer.numC_Trans,
                self.img_view_transformer.D,
                H,
                W,
            )
...
class BEVDetSequentialES(BEVDetSequential):
    ...
    def extract_img_feat(self, img, img_metas):
        ...
        rots, trans, intrins, post_rots, post_trans = extra
        bev_feat_list = []
        for img, _, _, intrin, post_rot, post_tran in zip(
            imgs, rots, trans, intrins, post_rots, post_trans
        ):
            tran = trans[0]
            rot = rots[0]
        ...
        if self.before and self.pre_process:
            bev_feat_list = [
                self.pre_process_net(bev_feat)[0] for bev_feat in bev_feat_list
            ]
...
from mmdet3d.models.necks.hybird_encoder import feats_to_img
...
class BEVDepth(BEVDepth_Base, BEVDet):
    def extract_img_feat(self, img, img_metas):
        """Extract features of images."""
        x = self.image_encoder(img[0])
        x, depth, _ = self.img_view_transformer([x] + img[1:])
        x = self.bev_encoder(x)
...
class BEVDepth4D(BEVDepth_Base, BEVDetSequentialES):
    def extract_img_feat(self, img, img_metas):
        ...
        for img, _, _, intrin, post_rot, post_tran in zip(
            ...
            depth_feat = self.img_view_transformer.se(depth_feat, cam_params)
