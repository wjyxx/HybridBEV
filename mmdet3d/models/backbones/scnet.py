...
class KD_SCNet(nn.Module):
    def __init__(self, input_channels, out_channels, num_layers):
        super().__init__()
        self.layers = self.make_layer(
            SCBottleneck, input_channels, out_channels, num_blocks=num_layers
        )

    def make_layer(
        self, block, input_channels, out_channels, num_blocks, norm_layer=nn.BatchNorm2d
    ):

        layers = []

        for i in range(1, num_blocks):
            layers.append(block(input_channels, out_channels, norm_layer=norm_layer))
